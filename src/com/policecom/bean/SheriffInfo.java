...
public class SheriffInfo {
		...
		public static void loginResultAnalysis(String st,MyApplication myapp){
			...
			if(st.equals("0") ||st.equals("1") ||st.equals("2")){
				...
				myapp.S_INFO.set_USER_ID(Integer.parseInt(sst[0]));
				myapp.S_INFO.set_USER_NAME(sst[1]);
				myapp.S_INFO.set_USER_NUMBER(sst[2]);
				
				myapp.S_INFO.set_USER_PWD(sst[3]);
				myapp.S_INFO.set_USER_PHONE(sst[4]);
				myapp.S_INFO.set_USER_OFFICENUM(sst[5]);
				
				myapp.S_INFO.set_USER_ONDUTYSIGN(sst[6]);
				myapp.S_INFO.set_USER_ONWEEK1(sst[7]);
				myapp.S_INFO.set_USER_ONWEEK2(sst[8]);
				
				myapp.S_INFO.set_USER_ONDUTYTIME1(sst[9]);
				myapp.S_INFO.set_USER_ONDUTYTIME2(sst[10]);
				myapp.S_INFO.set_USER_ADDRESS(sst[11]);
				myapp.S_INFO.set_USER_OID(sst[12]);
			}
		}
}
